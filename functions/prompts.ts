...
export const getSearchPrompts = (storyCore: string, options: StoryOptions): { role: string; content: string; }[] => {
    const system = `你是一位AI研究员，专门为网络小说作家提供市场分析和创意方向。
你的任务是根据用户的核心创意，进行一次深入的研究分析，并以清晰的Markdown格式输出你的思考过程。
你的输出应该是纯文本，不包含任何JSON代码块。`;

    const user = `### 任务：为以下故事核心进行深度研究和分析
...
### 任务流程
请用以下Markdown结构清晰地展示你的创作思路。

#### 市场与趋势分析 (模拟联网搜索)
*   **潜力与定位**: 分析此核心创意的市场潜力和最适合的细分赛道。
*   **热门元素融合**: 基于当前的热门趋势，提出3-5个可以与核心创意巧妙融合的元素，并简述融合方式。
*   **风险与规避**: 指出这个创意可能遇到的“毒点”或套路陷阱，并提出规避建议。
*   **风格对标**: 简要分析模仿 **${options.authorStyle}** 风格的关键要点和挑战。

#### 核心创作思路
*   **世界观构思**: 提出一个独特且吸引人的世界观初步构想。
*   **角色设计**: 阐述主角、关键配角和核心反派的设计思路，重点突出他们的核心矛盾和人物关系。
*   **情节框架**: 构思一个包含“开端-发展-高潮-结局”的完整故事主线框架，并点出几个关键的转折点或悬念。
...
export const getStoryOutlinePrompts = (storyCore: string, researchText: string, options: StoryOptions): { role: string; content: string; }[] => {
    const system = `你是一位顶级网络小说作家和世界观架构师。
你的任务是根据用户的核心创意和一份研究分析报告，将所有想法整合并输出一份**完整、详细、结构化**的创作大纲。
你的输出必须遵循两个步骤：首先是你基于研究报告的最终构思，然后是一个单独的、格式严谨的JSON对象。`;

    const user = `### 任务：根据研究报告生成完整创作大纲
...
**第一步：最终构思**
在你生成最终的JSON大纲之前，请先用以下Markdown结构，简要总结你根据上述研究报告确定的最终创作方向。
...
**第二步：输出JSON大纲**
在思考过程之后，请严格按照以下JSON结构输出完整的大纲，并使用 \`[START_OUTLINE_JSON]\` 和 \`[END_OUTLINE_JSON]\` 将其包裹。
